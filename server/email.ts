...
StrataGuard System
  ...
  const emailRequest: EmailRequest = {
    ...
    metadata: {
      ...
      adminUserId: (adminUser as { id?: string }).id, // If adminUser has an ID, include it
      recipientType: 'admin/council',
    },
    // Idempotency key can be generated by the service or explicitly here:
    idempotencyKey: `violation-${violation.uuid || violation.id}-pending-approval-admin-${(adminUser as { id?: string; email: string }).id || adminUser.email}`
  };
