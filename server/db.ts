...
// Configure connection pool with more relaxed timeout settings
const pool = new Pool({ 
...
pool.on('acquire', () => {
  logger.debug(`Connection acquired from pool (total: ${pool.totalCount}, idle: ${pool.idleCount})`);
});

pool.on('remove', () => {
  logger.debug(`Connection removed from pool (total: ${pool.totalCount}, idle: ${pool.idleCount})`);
});
