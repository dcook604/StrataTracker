...
# Add shell options for better error handling
set -euo pipefail
IFS=$'\n\t'
...
check_database_status() {
    ...
    local DB_CONTAINER
    DB_CONTAINER=$(docker ps --format "{{.Names}}" | grep -E "(stratatracker.*db|db.*stratatracker)" | head -1) || true
...
stop_existing_processes() {
    ...
    BACKEND_PIDS=$(pgrep -f "npm run dev:backend" 2>/dev/null) || true
    NODEMON_PIDS=$(pgrep -f "nodemon.*server" 2>/dev/null) || true
    ...
    if [ -n "${FRONTEND_PIDS}" ] || [ -n "${VITE_PIDS}" ] || [ -n "${BACKEND_PIDS}" ] || [ -n "${NODEMON_PIDS}" ]; then
        ...
        REMAINING_PIDS=$(pgrep -f "npm run dev\|vite\|nodemon.*server" 2>/dev/null) || true
...
find_available_port() {
    ...
    for i in $(seq 0 "${MAX_PORT_SEARCH_ATTEMPTS}"); do
        ...
        if ! lsof -i ":${CURRENT_PORT}" | grep LISTEN > /dev/null 2>&1; then
